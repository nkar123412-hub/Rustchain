...
"""Dependency-light RustChain tools for Agno and legacy Phidata agents."""
...
class RustChainAgnoTools:
    """Expose RustChain public reads as plain methods and an Agno Toolkit."""
    ...
    def _get_json(
        ...
        request = Request(
            url,
            headers={"Accept": "application/json", "User-Agent": "rustchain-agno"},
        )
        try:
    ...
    def _load_toolkit_class() -> type[Any]:
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    "Agno or Phidata is required for adapters: "
                    "pip install agno or pip install phidata"
                ) from exc

    def as_agno_toolkit(self) -> Any:
        """Create an Agno/Phidata Toolkit registering the four public actions."""
        toolkit_cls = self._load_toolkit_class()
        outer = self

        class RustChainToolkit(toolkit_cls):  # type: ignore[misc, valid-type]
            def __init__(self) -> None:
                super().__init__(name="rustchain")
                self.register(outer.check_balance)
                self.register(outer.list_bounties)
                self.register(outer.get_node_health)
                self.register(outer.get_current_epoch)

        return RustChainToolkit()
